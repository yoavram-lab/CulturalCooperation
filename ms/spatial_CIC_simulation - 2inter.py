...
def individualInteraction_behavior_stag_hunt(mySub: tuple[int, int], neibSub: tuple[int, int],
                                             lattice_ab: np.ndarray, params: dict) -> tuple[float, float]:
    '''
    returns the fitness delta of the two interactors according to the cost
    and benefit of the interactions (B, b and c)

    :return: tuple of the fitness delta of the two interactors
    '''
    c = params['c']
    b = params['b']
    big_b = params.get('B', 0)
    ...
    # Stag hunt game
    both_cooperative = my_ab * neib_ab

    # fitness calculation according to cooperation
    myfi = both_cooperative * big_b + (1-both_cooperative) * neib_ab * b - myC
    neibfi = both_cooperative * big_b + \
        (1-both_cooperative) * my_ab * b - neibC
    ...
    for it in range(K):
        ...
        for mySub in allSubs:
            ...
            if np.random.random() < InterProb:
                ...
                j = mySub[1]
                myId = i*N+j
                neibIds = neighbours(lattice_Ids, i, j, nei,
                                     nei).flatten().tolist()
                ...
                myfi, neibfi = individualInteraction_behavior_stag_hunt(
                    mySub, neibSub, lattice_ab, params)
                ...
                neibId = np.random.choice(neibIds)  # choose a partner
                neibSub = (neibId//N, neibId % N)
                individualInteraction_Infection(
                    mySub, neibSub, lattice_ab, params)
...
def selection_allpop(lattice_ab, lattice_fi, params):
    ...
    indices = np.random.choice(
        np.arange(N**2), size=N**2, replace=True, p=probs)
...
c = 0.02
b = 1.3
big_b = 3         # benefit of cooperation
Tb = 0.435
Ta = 0.4
