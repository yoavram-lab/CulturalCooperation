...
\begin{figure}[h]
  \centering       
    ...
    \caption{\textbf{Stable and unstable equilibrium dynamics} 
...
The equilibrium where only allele $M$ is present is  $\vec{\hat{p}^*} = (\hat{p}^*, 1-\hat{p}^*, 0, 0)$, where
...
We did a full analysis without SymPy for the special case when $v=1$ (see Appendix~\ref{sec:appendixC}).
We will use SymPy\citep{Meurer2017}, a Python library for symbolic mathematics to simplify $R(1)$ in the general case when $0<v\leq 1$. 
\begin{equation} \label{eq:value_at_1}
 ...
 & + \frac{c v \hat{p}^*[T_Ac\hat{p}^{*2}(1-v)+T_Ac\hat{p}^*(v-1)-T_A\hat{p}^*(1-c)+T_A]}{b\hat{p}(b\hat{p}^*-2\hat{p}^*+2)+c\hat{p}^*(c\hat{p}^*-2)+1} \\
  ...
  \end{aligned}
\end{equation}
As explained, we should find when $R(1)<0$. We know that when $\alpha_1 = \alpha_2$, $R(1)=0$. 
...
Thus, $R(1)<0$ if and only if $\alpha_2<\alpha_1$. This is a sufficient condition  for external instability.
