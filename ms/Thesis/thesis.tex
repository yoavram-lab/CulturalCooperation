...
Similar to dominant strategies bringing forth Nash equilibria when games are repeated, strategies in replicator dynamics can become evolutionary stable. Such strategies are called \emph{Evolutionarily Stable strategy (ESS)}.
Such strategies cannot be invaded by any other strategy that is initially rare. Evolutionarily stable strategy maximizes the expected fitness of its chooser and therefore, causing maximum mean fitness.
...
The answer to the question above is that natural selection usually does not lead to an increase in the average mean fitness of the population. Therefore, it is not generally true that natural selection does operate as to maximize the average individual fitness resulting in an ESS.
\citet{sacks1967stable} even showed that natural selection at a diploid locus model when relative fitnesses of the different genotypes are functions of gene frequency leads to stable equilibrium gene frequency that corresponds to minimum mean fitness.
Instead of searching evolutionarily stable strategy, we focus on \emph{Evolutionary Genetic Stblility (EGS)}\citep{lessard1990evolutionary}. EGS is when the frequency of types (genotype, phenotype etc.) stays the same.
...
\emph{External stability}\citep{Liberman1986modifiers,Altenberg2017} is a local stability of the equilibrium to invasion by the modifier allele m. One of the main questions in the evolution of cooperation is under what conditions such invasions are possible, or in other words under what conditions the system is external unstable.
...
%%% Figure: model illustration - basic model
\begin{figure}[pt]
  \centering
...
In this section, we use the model described in \autoref{section:basicmodel}. 
Here we assume a coexistence of both phenotypes (cooperators and defectors) and that the frequency of each phenotye stays the same each generation.
Henece, we assume that the we start from one of the equilibrium.
In addition, we assume that the equilibria is stable.  Denote the frequency of cooperators in the population in $\hat{p}^*$ and frequency of defectors $1-\hat{p}^*$.

Starting from the assumption described above we want to investigate the evolution of interaction-transmission association $\alpha$.
We assume that the initial population has interaction-transmission association $\alpha_1$. 
We want to answer the question what would happen to cooperators frequency if a new mutation would occur and change the interaction-transmission association to a different value $\alpha_2$ without direct effect on fitness.
We can think of this model as an invasion model where the invadors are the individuals with interaction-transmission association $\alpha_2$. \autoref{fig:invasion_illustration} illustrates the invasion.

%%% Figure: model illustration - invasion model
\begin{figure}[pt]
  \centering
  ...
  A mutation that has effect only the interaction-transmission association tries to invade. }
...
%\end{equation}
where $x=\hat{p}_1+\hat{p}_3$ is the total frequency of the cooperative phenotype $A$, and $\bar{w} = 1 + (b-c) x$ is the population mean fitness.% and $\vec{\dot{p}} = (\dot{p}_1,\dot{p}_2,\dot{p}_3,\dot{p}_4)$.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=1\textwidth]{../PRSB_figures/fig_8.pdf}
  \caption{
...
This would make the model more realistic and more complicated to solve. To overcome the math complication, a numerical simulations can be used. 
The model can be further expand by changing the game to Stag Hunt game. \citet{tomasello2012two} claim that Stag Hunt game can better explain evolution of cooperation in humans. 
Humans often collobrate in Stag Hunt type situations in which all participants had alternatives but anticipated an even greater benefit from
successful capturing of the stag\citep{mitani2012evolution}.
...
We did it in the general case (starting from \autoref{eq:value_at_1_derivative}).
