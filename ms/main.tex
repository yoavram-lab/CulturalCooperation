...
We are looking for the equilibrium $p^*$ which happens when $\hat{p}' = \hat{p}$ and we get
\begin{equation} \label{eq:oblique_and_vertic_result}
  \hat{p}^* =  \frac{\alpha bvT_A + cvT_B + T_A-T_B}{\alpha bv\cdot(T_A-T_B)- b\cdot(T_A-T_B) - cv\cdot(T_A-T_B) + c\cdot(T_A-T_B)}
\end{equation}
