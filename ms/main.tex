...
\begin{equation} \label{eq:appendix_c_taylor}
  f(\hat{p}) = \bar{w}(\hat{p}'-\hat{p}) = f(\hat{p}^*) + f'(\hat{p}^*)(\hat{p}-\hat{p}^*) + O((\hat{p}-\hat{p}^*)^2)
\end{equation}
Where $O((\hat{p}-\hat{p}^*)^2)$ is big O notation. Denote, $\epsilon = \hat{p}-\hat{p}^*$ and $\epsilon'=\hat{p}'-\hat{p}$.
\begin{equation} \label{eq:appendix_c_taylor0}
  f(\hat{p}) = \bar{w}\epsilon' = f(\hat{p}^*) + f'(\hat{p}^*)\epsilon + O(\epsilon^2)
\end{equation}
Since $\hat{p}^*$ is an equilibrium $f(\hat{p}^*)=0$ we get
\begin{equation} \label{eq:appendix_c_taylor0}
  f(\hat{p}) = \bar{w}\epsilon' = f'(\hat{p}^*)\epsilon + O(\epsilon^2)
\end{equation}
By definition of big-O, if $f’(\hat{p}*)$ is negative, then there exists some $\epsilon$ (i.e. some neighbourhood of $\hat{p}^*$) for which $f’(\hat{p}^*) + O(\epsilon)$ is also negative.
Therefore,
\begin{equation} \label{eq:appendix_c_taylor_is_negative}
  \bar{w}\epsilon' = f'(\hat{p}^*)\epsilon + O(\epsilon^2) < 0
\end{equation}
In addition, we know that $f(\hat{p}^*)=0$ therefore, there exist some $\epsilon$ for which $f(\hat{p}) > - \delta$ for some $\delta > 0$. 
We choose delta to be $\delta = \frac{1}{\bar{w}}$, therefore  $f(\hat{p})=\bar{w}\epsilon'>-\frac{1}{\bar{w}}$. Therefore,
\begin{equation} \label{eq:appendix_c_taylor_is_negative_and_bound_by_minus1}
  -\frac{1}{\bar{w}} <\bar{w}\epsilon' = f'(\hat{p}^*)\epsilon + O(\epsilon^2) < 0
\end{equation}

Let's start simplify
\begin{equation} \label{eq:appendix_c_taylor_is_negative}
  -\frac{1}{\bar{w}} < \bar{w}\frac{\epsilon'}{\epsilon} < 0
\end{equation}
Since $\bar{w}>0$ we get that
\begin{equation} \label{eq:appendix_c_condition_for_positive_epsilon}
  \begin{aligned}
  -1 < \frac{\epsilon'}{\epsilon} & = \frac{\hat{p}'-\hat{p}}{\hat{p}-\hat{p}^*} = \frac{\hat{p}'-\hat{p}^*+\hat{p}^*-\hat{p}}{\hat{p}-\hat{p}^*} \\
  & \frac{\hat{p}'-\hat{p}^*}{\hat{p}-\hat{p}^*} - 1< 0
  \end{aligned}
\end{equation}
From eq.~\ref{eq:appendix_c_condition_for_positive_epsilon} we get that
\begin{equation} \label{eq:appendix_c_final_condition_for_positive_epsilon}
  \begin{aligned}
    -1 < \frac{\hat{p}'-\hat{p}^*}{\hat{p}-\hat{p}^*} < 1
  \end{aligned}
\end{equation}
We can apply absoulte value
\begin{equation} \label{eq:appendix_c_final_condition_for_positive_epsilon}
  \begin{aligned}
...
Therefore, if $f'(\hat{p}^*)<0$ there exist some neighbourhood of $\hat{p}^*$ where $\hat{p}'$ is closer to $\hat{p}$, which means that $f'(\hat{p}^*)<0$ is sufficient condition for stability of $\hat{p}^*$.

Now let's assume by contradiction that $\hat{p}^*$ is also locally stable when $f'(\hat{p}^*) > 0$.
When $f'(\hat{p}^*) > 0$ there exist some neighbourhood $\epsilon$ of $\hat{p}^*$ in which $f'(\hat{p})>0$ for every $\hat{p}$ in the neighbourhood.

If $\hat{p} > \hat{p}^*$ we get:
\begin{equation} \label{eq:appendix_c_1}
  \begin{aligned}
    f(\hat{p}) = w\epsilon' > f(\hat{p}^*) = 0
  \end{aligned}
\end{equation}
From eq.~\ref{eq:appendix_c_1} we can see that $\hat{p}' > \hat{p} > \hat{p}^*$ and therefore, $\hat{p}^*$ is not stable.

If $\hat{p}<\hat{p}^*$ we get:
\begin{equation} \label{eq:appendix_c_2}
  \begin{aligned}
    f(\hat{p}) = w\epsilon' < f(\hat{p}^*) = 0
  \end{aligned}
\end{equation}
From eq.~\ref{eq:appendix_c_1} we can see that $\hat{p}' < \hat{p} < \hat{p}^*$ and therefore, $\hat{p}^*$ is not stable.
%TODO: Add proof for f'(p)=0

We saw that if $f'(\hat{p}^*)<0$ then $\hat{p}^*$ is stable, and if $f'(\hat{p}^*)>0$ then $\hat{p}^*$ is unstable.
Therefore, $f'(\hat{p}^*)$ is sufficient and necessary condition for equilibrium stability.
