...
If the initial frequency of cooperation is very close to zero, $\tilde{p}_0 \approx 0$, then its frequency will increase if 
\begin{equation} \label{eq:unequal_transmission_from_rarity}
\begin{aligned}
c < \gamma_1 = \frac{b \alpha T_A + (T_A - T_B)}{1-T_B} \;.
\end{aligned}
\end{equation} 
\end{corollary}

Note that the right-hand side equals $\gamma_1$.
...
\item \emph{Stable Coexistence}: 
    if \emph{(v)} $T_A < T_B$ and $v>\hat{v}$ and $\hat\gamma_2 < c < \hat\gamma_1$. 
\item \emph{Unstable coexistence}:
    if \emph{(vi)} $T_A > T_B$ and $\hat\gamma_1 < c < \hat\gamma_2$.
\end{enumerate}

\end{result}
These conditions are illustrated in \autoref{fig:result3}.

\emph{//additional corollaries//}
...
Cost boundaries $\hat\gamma_1$ and $\hat\gamma_2$ (\autoref{eq:cost_boundaries_v}) are represented by the solid and dashed lines. 
(\textbf{a}) $b=1.2$, $T_A = 0.5$, $T_B = 0.4$, $\alpha = 0.4$.
(\textbf{b}), $b=2$, $T_A=0.5$, $T_B = 0.7$, $\alpha = 0.7$.
	}
=======
In (\textbf{left}) the horizontal bias $T_A-T_B$ is negative, in (\textbf{right}) the horizontal bias is positive.
The vertical transmission rate $v$ is on the x-axis; the cost boundaries $\hat\gamma_1$ and $\hat\gamma_2$ (\autoref{eq:cost_boundaries_v}) are the solid and dashed lines. 
In (\textbf{left}), $b=2$, $T_A=0.5$, $T_B = 0.7$, $\alpha = 0.7$ and (\textbf{right}) $b=1.2$, $T_A = 0.5$, $T_B = 0.4$, $\alpha = 0.4$.}
>>>>>>> f639c96... Update figure 4 - from a,b to left and right
    \label{fig:result3}
\end{figure}
