...
The blue curve is above the black dashed line$\hat{p}<\hat{p}^*$, so that the frequency increases towards $\hat{p}^*$. The blue curve is below the orange line when $\hat{p}>\hat{p}^*$, so that the frequency decreases towards $\hat{p}^*$.
}
