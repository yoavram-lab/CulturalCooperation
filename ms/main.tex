...
Table~\ref{table:1} contains the probability of first interactor to be A following interaction by interaction type.
...
\end{aligned}
\end{equation}
where fitness values are taken from table~\ref{table:1} and $\bar{w} = \hat{p} w_A + (1-\hat{p}) w_B = 1 + \hat{p}(b-c)$ is the population mean fitness.
...
\tilde{p} & > \frac{c\cdot(1-T_B)+T_B-T_A}{b\cdot(T_A-T_B)}
\end{split}
\end{equation}
Hence, cooperation will take over the population when when eq.~\ref{eq:horizontal_transmission_from_population_reorder} is satisfied, that is if $\tilde{p}$ is above the threshold on the RHS. 
When $\tilde{p}$ is equal to eq.~\ref{eq:horizontal_transmission_from_population_reorder} RSH the frequency of phenotype A do not change. Denote $\tilde{p}^{*}$:
\begin{equation} 
\begin{split} \label{eq:horizontal_transmission_from_population_equilibrium}
\tilde{p}^{*} & = \frac{c\cdot(1-T_B)+T_B-T_A}{b\cdot(T_A-T_B)} .
\end{split}
\end{equation}
Therefore, $\tilde{p}^{*}$ is the third equilibrium in this system.
Specifically, a sufficient condition for cooperation fixation is found by setting the the equilibrium $\tilde{p}^*$  to be negative:
\begin{equation} 
...
So far, we assumed that $v=1$. The next section discuss the effect of oblique transmission: transmission from non-parental adults.

\paragraph*{Without vertical transmission, $v = 0$.}

In this case eq.~\ref{eq:horizontal} becomes $\hat{p}=p$ and eq.~\ref{eq:nextgen_parents} becomes:
\begin{equation} 
