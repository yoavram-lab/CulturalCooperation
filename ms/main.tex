...
\textbf{Result 3: Oblique and vertical transmission of cooperation with symmetric horizontal transmission.}

Assuming equal horizontal transmission, i.e. $T_A = T_B = T$, and following the above procedure, we have
\begin{equation} \label{eq:equal_horizontal_transmission}
%\hat{p}' - \hat{p} =  \hat{p} ^2(-\alpha bvT + cv(1-T)) + \hat{p} (\alpha bvT -cv(1-T))
\bar{w} (\hat{p}' - \hat{p}) =  \hat{p} ^2(-\alpha bvT + cv(1-T)) + \hat{p} (\alpha bvT -cv(1-T))
\end{equation}

It is easy to see that $\hat{p}^*_0 =  0$ and $\hat{p}^*_1 = 1$ are equilibria of eq.~\ref{eq:equal_horizontal_transmission}, that is, both extinction and fixation of the cooperator phenotype are equilibria.

%TODO fix the following, you forgot wbar in eq:equal_horizontal_transmission
...
Next, we study the general case where $T_A \neq T_B$. We have
%TODO check again, I think you forgot wbar here, too
\begin{equation} \label{eq:general_case_polynomial}
  f(\hat{p}) = \hat{p}'-\hat{p} = \beta_1 p^3 + \beta_2 p^2 + \beta_3 p
\end{equation}
where 
...
$f(\hat{p})$ is a cubic, and its three roots are the equilibria of the system, i.e. the solutions for $\hat{p}'=\hat{p}$.
As $\hat{p}^*_0 = 0 $ and $\hat{p}^*_1 = 1$ are equilibria, there is only one equilibrium left, $\hat{p}^*_2$, which solves $f(\hat{p})/\big(\hat{p}(1-\hat{p})\big)=0$. Solving this we find that
...
$\hat{p}^*_2$ is a legitimate equilibrium if $0<\hat{p}^*_2<1$.  
To determine if $\hat{p}^*_2$ is legitimate, we need the derivative of $f(\hat{p})$,\begin{equation} \label{eq:derivative_of_phattag-phat2}
  f'(\hat{p}) = 3\beta_0 \hat{p}^2 + 2\beta_1\hat{p}^2 + \beta_3.
 \end{equation}
First, we assume that $T_A>T_B$, and therefore $\beta_1<0$, because $c(1-v)-b(1-\alpha v)<0$ and $c<b$ $v<1$. % why?
In this case $f(\hat{p})$ goes to infinity when $\hat{p}$ goes to infinity. 
Therefore, if $\hat{p}^*_2$ is positive then $f'(0)= \beta_3$ must be negative, and
% TODO remove
%\begin{equation} \label{eq:derivative_of_phattag-phat_p=0}
%  f'(0) = \beta_3
% \end{equation} 
$\hat{p}_2^*>0$ when $\beta_3<0$. In addition, $\hat{p}^*_0=0$ is stable when $|f'(0)|<1$ therefore, when $-1<\beta_3<1$. We can show that 
\begin{equation} \label{eq:beta3-condition}
  \beta_3 = \alpha bvT_A - cv(1-T_B) + (T_A-T_B) > -cv(1-T_B) > -1
\end{equation}
Therefore, when $\hat{p}^*_2>0$ we get that $-1<\beta_3<0$ and $\hat{p}^*_0=0$ is locally stable. 

Next we would like to find when $\hat{p}^*_2<1$.
If $T_A>T_B$ like before then the polynomial goes to minus infinity when $\hat{p}$ goes to infinity.
