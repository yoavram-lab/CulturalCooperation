...
%\end{equation}
where $x=\tilde{p}_1+\tilde{p}_3$ is the cooperator frequency, and $\bar{w} = 1 + (b-c) x$ is the population mean fitness.% and $\vec{\tilde{p}} = (\tilde{p}_1,\tilde{p}_2,\tilde{p}_3,\tilde{p}_4)$.
...
where the elements of $\mathbf{L}^*_{ex}$ are derived from \autoref{eq:next_gen_p_1}.
...
Note that $\mathbf{L}^*_{ex}$ is positive, and due to the Perron-Frobenius theorem, its leading eigenvalue is real and positive.
If the leading eigenvalue is less than 1, then the equilibrium $\vec{\tilde{p}^*}$ is externally stable and allele $m$ cannot invade the population of allele $M$. 
...
The characteristic polynomial $R(\lambda)$ is a quadratic with a positive leading coefficient. Therefore, $R(\infty)=R(-\infty)>0$.
Hence, the leading eigenvalue is greater than one, implying external stability, if and only if $R(1)>0$ and $R'(1)>0$.
Thus, a sufficient condition for instability is $R(1)<0$.
Using \emph{SymPy} \citep{Meurer2017}, a Python library for symbolic mathematics, we find that $R(1)<0$ if and only if $\alpha_2 < \alpha_1$.
That is, if the social association $\alpha_2$ of the invading modifier allele $m$ is less than $\alpha_1$ of the resident $M$, then invasion will be successful.
...
\begin{figure}[p]
  \centering
   \includegraphics[scale=0.5]{{Ta=Tb_b=1.3_globalSelection}.pdf}
    ~
    \includegraphics[scale=0.56]{{Ta=0.4_Tb=0.3-0.5_b=1.3_globalSelection_withSmallBox_a}.pdf} \\
    ~
...
\begin{figure}[b]
  \centering
   \includegraphics[scale=0.4]{Frequency_of_invadors.eps}
    ~
    \includegraphics[scale=0.4]{Frequency_of_cooperation_after_invasion.eps}\\ 
  \caption{
  ...
  \textbf{(a)} describes the frequency of the invadors. 
  \textbf{(b)} describes the frequency of cooperators after invasion. The frequency of cooperators decreases towards new stable frequency $\hat{p}_2^*$ which described by the black dashed line. 
  $\hat{p}_2^*$ is the equilibrium described in \autoref{eq:general_equilibrium} with soical association $\alpha=\alpha_2$.
