...
Therefore, if $\hat{p}^*_2$ is positive then $f'(0)$ must be negative % are you sure? I think it must be positive, because i think that the two extremes of a cubic must go in opposite directions DC: Yest, but I've fixed in the line before, there was minus sign missing
...
Hence, if $\hat{p}^*_2<1$ then $f'(1)$ must be negative % again, i think that the two extremes of a cubic must go in opposite directions. DC: I've double checked and I'm pretty sure my results are correct 
...
\textbf{Result 4.3}: if $T_B>T_A$, then $\hat{p}^*_2$ is a legitimate equilibrium when $\beta_1>\beta_3>0$. % DC: I didn't show it but it applies
