...
% Abstract
\begin{abstract}
We construct models for the cultural evolution of cooperation in a well-mixed population with vertical, horizontal, and oblique transmission. Conditions are found for fixation of cooperation, fixation of defection, and stable coexistence between cooperation and defection, and comparisons are drawn with Hamilton's rule. A spatial model is constructed and its results are compared to the well-mixed model.
We show that assortment between cooperation and horizontal transmission facilitates the evolution of cooperation and increases the population mean fitness. 
...
\end{equation}
In this case $\varphi$ takes the role of the relatedness $r$ in \autoref{eq:hamilton_rule}, but the effective benefit $b\cdot \varphi$ is further reduced by $(1-\varphi)/\varphi$.

Non-vertical cultural transmission may be either viewed as horizontal or oblique: horizontal transmission occurs between individuals from the same generation, while oblique transmission occurs  to  offspring from the generation to which their parents belong. 
...
\citet{lewin2017microbes} and \citet{lewin2020rockpaperscissors} have demonstrated that non-vertical transmission, mediated by microbes that manipulate their host's behavior, can help to explain the evolution of cooperative behavior. Some of their analysis can be applied to cultural transmission, because models of cultural transmission are mathematically similar to those for transmission of infectious diseases~\citep{cavalli1981cultural}.
Importantly, their results depended on non-vertical transmission of the cooperation-determining microbes.

\citet{woodcock2006significance} showed the importance of non-vertical transmission for the evolution of cooperation. In their model the individuals interact according to standard prisoner dilemma game in which two individuals who interact can cooperate or not.
In their work they do not link between interaction and transmission. Their model illustrates that it
is possible to sustain altruistic behaviour at the level of culture for a significant
period of time in the absence of any direct biological propensity favouring this
behaviour.
...
%\item \emph{Fixation of either phenotype depending on initial frequency}:
%	if \emph{(viii)}  $T_A>T_B$ and $\beta_3<\beta_1<0$.
%\end{enumerate}
...
\item \emph{Fixation of either phenotype depending on initial frequency}:
  if \emph{(viii)}  $T_A>T_B$ and $c > \hat{\gamma_2}$ and $c > \hat{\gamma_1}$ and $v>\hat{v}$.
\end{enumerate}
\end{result}
...
We saw that it is likely to find configuration of parameters that results coexistence as can be seen in \autoref{fig:results2}b. In figure \autoref{fig:results2}b the yellow area in which coexistence occur has horizontal bias that favor defection ($T_B>T_A$) and the cost is relatively small. 
...
Microbes can also be transferred vertically, from parent to offspring, and % yr: cite 10.1126/science.aat7164
...
Our results highlight another possibility for assortment, namely, individuals interacting at rate $\alpha$ with their cultural partners, resulting in horizontal transmission.
