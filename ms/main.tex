...
The effect of vertical/oblique transmission is more complex. 
When there is horizontal transmission bias for cooperation ($T_A>T_B$), increasing the rate of oblique transmission ($1-v$) will facilitate the evolution of cooperation (\autoref{fig:result3}ac). However, when the bias is for defection ($T_A<T_B$), high rates of vertical transmission ($v$) are advantageous for cooperation. 
Nevertheless, to reach fixation of cooperation in the population, there must be a sufficient rate of vertical transmission ($v>\hat v$, Corollary~\autoref{corollary:vhat}).
% TODO carry on summarizing corollaries here

The results that we derived for a deterministic well-mixed model provide a good approximation to results of simulations of a complex stochastic spatial model  (\autoref{fig:spatial}).
