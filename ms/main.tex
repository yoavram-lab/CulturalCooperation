...
A similar analysis can be made when $T_A>T_B$. $\beta_3>0$ requires $\hat\gamma_1 > c$. 
For $\beta_1<\beta_3$ we need $c\big[v(1-T_B) + (1-v)(T_A-T_B)\big] < bv\alpha T_B + (1+b)(T_A-T_B)$.
Thus for $\beta_1<\beta_3$ we need $v > \hat v$ and $c < \hat\gamma_2 $ or $v < \hat v$ and $c > \hat\gamma_2$, but $\hat{v}<0$ when $T_A > T_B$ 
and therefore, we get that $\beta_1<\beta_3$  if $c < \hat\gamma_2$. Similarly, we get that $\beta_3<\beta_1$ if $c > \hat{\gamma_2}$.
