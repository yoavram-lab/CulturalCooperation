...
We can find the third equilibrium by finding wehen eq.~\ref{eq:general_case_polynomial} is equal to zero and we get
\begin{equation} \label{eq:oblique_and_vertic_result}
  \hat{p}^*_2 =  \frac{\alpha bvT_A + cvT_B + T_A-T_B-cv}{c(1-v)\cdot(T_A-T_B) -b (1-\alpha v) \cdot(T_A-T_B)}
\end{equation}
Next we would like to learn under what conditions $\hat{p}^*_2$ is locally stable. 
For simplicity let's assume that $T_A > T_B$ and we will check when $\hat{p}^*_0 \, \hat{p}^*_1$ are stable.
...
\begin{equation} \label{eq:derivative_of_phattag-phat2}
 ...
 \end{equation}
 When $\hat{p} = \hat{p}^*_0 = 0$ we get
 \begin{equation} \label{eq:derivative_of_phattag-phat_p=0}
  f'(0) = \beta_3
 \end{equation}
 Therefore, $\hat{p}^*_0$ is stable when
 \begin{equation} \label{eq:p=0_stability_condition}
  f'(0) = \beta_3 = \alpha bvT_A - cv(1-T_B) + (T_A-T_B) < 0
 \end{equation}
 And $\hat{p}^*_1=1$ is stable when
 \begin{equation} \label{eq:p=1_stability_condition}
  f'(1) =3\beta_1 + 2\beta_2 +\beta_3 = cv + (T_A-T_B)(b-c) - T_B(1+\alpha bv)  + T_A(1-cv) < 0
 \end{equation}
