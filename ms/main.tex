...
\end{aligned}\end{equation}
Note that when $T_A>T_B$ then $\partial \hat a_2 / \partial v = b T_A (T_A-T_B) > 0$, and when $T_A<T_B$ then $\partial \hat a_1 / \partial v = b(1+b-c)T_B(T_A-T_B) < 0$.
Similarly, $\partial \hat a_1 / \partial T_A < 0$, $\partial \hat a_2 / \partial T_A < 0$, $\partial \hat a_2 / \partial T_B > 0$, and $\text{sign}(\partial \hat a_1 / \partial T_B) = \text{sign}\big(T_A (1+b-c) - (1-T_A)cv\big)$.
...
Note that increasing the horizontal transmission of cooperation, $T_A$, decreases both boundaries ($\partial \hat a_1 / \partial T_A, \partial \hat a_2 / \partial T_A < 0$), thus facilitating evolution of cooperation. 
Increasing the horizontal transmission of defection, increases one boundary, ($\partial \hat a_2 / \partial T_B > 0$).
So, if there is horizontal bias for cooperation ($T_A>T_B$), decreasing that bias will make it harder for cooperation to evolve.
However, if there is horizontal bias for defection ($T_A<T_B$) then the effect of further increasing the horizontal transmission of defection, $T_B$, can either increase or decrease the threshold $\hat\alpha_1$, making it harder or easier for cooperation to evolve, respectively, depending if horizontal transmission of cooperation ($T_A$) is high or low (i.e. if $T_A > cv/(1+b-c+cv)$ or below).

Interestingly, when there is horizontal transmission bias for cooperation ($T_A>T_B$), then vertical transmission hinders the evolution of cooperation ($\partial \hat a_2 / \partial v > 0$). In contrast, when there is horizontal transmission bias for defection ($T_A<T_B$), vertical transmission facilitates the evolution of cooperation ($\partial \hat a_1 / \partial v < 0$).
\\
