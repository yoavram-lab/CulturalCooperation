...
However, increasing the horizontal transmission of defection can either increase or decrease the boundary, making it easier or harder for cooperation to evolve, respectively,
depending if $T_A$ is high or low. Since $sign(\partial\gamma_1/\partial T_B) = sign(vb \alpha T_A+T_A-1)$ it will be easier to cooperation to evolve if $T_A > \frac{1}{1+\alpha b v}$ and harder otherwise.
...
\begin{figure}[h]
  ...
  Selection operates globally (see Fig X for results from a model with local selection).
  ...
  \textbf{(left)} Symmetric horizontal transmission, $T=T_A=T_B$.
  \textbf{(right)} Horizontal transmission rates $T_A=0.4$, $0.3<T_B<0.5$.
  }
  \label{fig:spatial}
\end{figure}


% TODO add figure of fluctuations in spatial model
% TODO add figure of local selection in spatial model
