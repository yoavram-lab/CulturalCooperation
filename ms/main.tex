...
\begin{aligned}
\hat\gamma_1 = \frac{b v \alpha T_A + (T_A - T_B)}{1-T_B}, \quad
...
With vertical, horizontal, and oblique transmission, the cultural evolution of a cooperator phenotype will follow one of the following scenarios depending on the cost boundaries $\hat\gamma_1$ and $\hat\gamma_3$ (\autoref{eq:cost_boundaries_v}):
...
\item \emph{Fixation of cooperation}: 
	...
	if \emph{(iiia)} $T_A<T_B$ and $v>\frac{T_B-T_A}{1-T_A}$ and $c < \hat\gamma_2$; or
	if \emph{(iiib)} $T_A<T_B$ and $v<\frac{T_B-T_A}{1-T_A}$ and $c > \hat\gamma_2$.
\item \emph{Fixation of the defection}: 
	if \emph{(iv)}  $T=T_A=T_B$ and $c > b\cdot \frac{\alpha T}{1-T}$; or 
	if \emph{(v)} $T_A>T_B$ and $\beta_1<\beta_3<0$; or 
	if \emph{(vi)} $T_A<T_B$ and $\beta_3<0$.
