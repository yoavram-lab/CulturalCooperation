...
Conditions are found for fixation and polymorphism of cooperation and defection. 
...
This condition cannot be formulated in the form of Hamilton's rule, $c<b \cdot r$, due to the effect of biased horizontal transmission.

Remarkably, stable polymorphism between cooperation and defection can be stable if horizontal transmission is biased for defection ($T_A<T_B$) and both the cost of cooperation and social association are intermediate (yellow areas in Figures~\ref{fig:result2} and~\ref{fig:result3}).
...
Nevertheless, when cooperation and defection coexist, social association is expected to be reduced by natural selection, leading to extinction of cooperation and decreased population mean fitness.
...
The conditions derived from our deterministic model provide a good approximation to outcomes of simulations of a complex stochastic model with population structurein which individuals can only interact with and transmit to their neighbors.
...
(\textbf{a-b})~The cost of cooperation $c$ is on the y-axis; the cost thresholds $\gamma_1$ and $\gamma_2$ (\autoref{eq:cost_thresholds_v_threshold}) are the solid and dashed lines, respectively. 
...
\begin{figure}[p]
  \centering       
	...
	(\textbf{a-b}) The cost of cooperation $c$ is on the y-axis and the cost thresholds $\gamma_1$ and $\gamma_2$ (\autoref{eq:cost_thresholds_v_threshold}) are represented by the solid and dashed lines, respectively. 
    (\textbf{c-d}) The social association $\alpha$ is on the y-axis and the social association thresholds $a_1$ and $a_2$ (\autoref{eq:boundries_assortative_meeting_general_case}) are represented by the solid and dashed lines, respectively. 
...
\begin{figure}[p]
  \centering
   \includegraphics[scale=0.5]{{Ta=Tb_b=1.3_globalSelection_a}.pdf}
    ...
    ~
    \includegraphics[scale=0.65]{{timeSereis_b=1.3_TA=0.4_TB=0.435_c=0.02_globalSelection_normalizeByNumInteractions_c}.pdf}
  \caption{
  \textbf{Evolution of cooperation in a structured population.}
	The expected frequency of cooperators in a structured population after 10,000 generations is shown (red for 0\%, green for 100\%) as a function of both the cost of cooperation ($c$) on the y-axis, and the symmetric horizontal transmission rate ($T=T_A=T_B$) on the x-axis of the left panel \textbf{(a)}, or the transmission bias $T_A-T_B$ on the x-axis of the right panel \textbf{(b)}.
  ...
  Figure \textbf{(b)} emphasizes an area in which neither phenotype is fixed throughout the simulation, maintaining stochastic polymorphism. 
  Figure \textbf{(c)} describes the frequency of cooperators and defactors over time
...
\begin{figure}[b]
  \centering
   ...
   \includegraphics[scale=0.5]{mean_fitness_of_invadors.eps}
  \caption{
  \textbf{Evolution of social association.} successful invasion of new modifier allele m; starting from stable polymorphism of cooperators and defactors with soical association $\alpha_1$. 
  The invadors have modifier allele m which cause different soical association value, $\alpha_2$ where $\alpha_2<\alpha_1$.
  \textbf{(a)} The blue curve describes the frequency of the invadors, starting from initial frequency of 1\%.
  ...
  \textbf{(b)} describes the mean fitness as a function of time (generations). 
  As the frequency of the invadors increases the mean fitness decreases. \\
  In both panels $T_A = 0.4$, $T_B = 0.7$, $b=1.3$, $c=0.05$, $\alpha_1 = 0.7$ and $\alpha_2 = 0.65$.
  }
  \label{fig:invasion}
\end{figure}
%TDODO: revisit this caption
