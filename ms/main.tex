...
\label{sec:cor_equal_transmission}
...
\end{aligned}
\end{equation}
Therefore with symmetric horizontal transmission ($T_A=T_B$), fixation of the cooperative phenotype ($\hat{p}=1$) occurs under the same condition as Corollary \ref{sec:cor_equal_transmission}, namely inequality~\ref{eq:equal_transmission}.
...
We find that increasing social association ($\alpha$) increases the opportunity for evolution of cooperation (corollary X, \autoref{fig:result2}c and \autoref{fig:result3}cd). 
Similarly, a horizontal transmission bias for cooperation ($T_A-T_B$) will also facilitate its evolution (corollary X, \autoref{fig:result2}}.
The situation with vertical/oblique transmission ($v$) is more complex. 
When there is horizontal transmission bias for cooperation ($T_A>T_B$), increasing the rate of oblique transmission ($1-v$) will facilitate the evolution of cooperation (\autoref{fig:result3}ac). However, when the bias if for defection ($T_A<T_B$), high rates of vertical transmission ($v$) are advantageous for cooperation. 
Nevertheless, to reach fixation of cooperation in the population, there must be a sufficient rate of vertical transmission ($v>\hat v$, Corollary~\autoref{sec:cor_vhat}).
% TODO carry on summarizing corollaries here

Our analytic conditions derived for a deterministic well-mixed model provides a good approximation to results of simulations of a complex spatial stochastic model  (\autoref{fig:spatial}).
In this spatial model, individuals can only interact with and transmit to their neighbors on a grid.
This demonstrates that social association between cooperation and transmission can arise in a structured population in which interactions are local.
