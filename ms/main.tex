...
% TODO YR revise; note that without vertical transmission TA-TB determines the outcome
...
Note that $\partial \hat a_2 / \partial v = b T_A (T_A-T_B)$. $\partial \hat a_2 / \partial v > 0$ when $T_A > T_B$ and therefore, as the vertical transmission $v$ \emph{increases} a greater value of social association is required for fixation of cooperation from rarity. %TODO YR revise
...
\subsection*{Modifiers of social association} 

We now analyze the evolution of social association under perfect vertical transmission, $v=1$.
We assume a population at a stable coexistence of the two phenotypes, cooperation $A$ and defection $B$, such that the frequency of $A$ among parents is $\tilde{p}^*$ (
\autoref{eq:vert_hori_equilibrium}).
Note that for stable coexistence to occur, we assume horizontal bias for defection, $T_A < T_B$, and an intermediate cost of cooperation, $\gamma_2 < c < \gamma_1$ (\autoref{eq:cost_boundaries})

The stable population mean fitness is $\bar{w}^* = 1 + \tilde{p}^*(b-c)$ (\autoref{eq:mean_fitness}), which is increasing in $\tilde{p}^*$. 
Therefore, to determine the effect of increasing $\alpha$ on the stable population mean fitness, we must analyze its effect on $\tilde{p}^*$, 
\begin{equation} \label{eq:mean_fitness_stable_coexistence_derivative}
  \frac{\partial \tilde{p}^*}{\partial \alpha} 
...
\end{equation} 
Therefore, $b T_A - c(1-T_B) - (T_B-T_A)>0$, and since $T_A<T_B$, the derivative $\partial \tilde{p}^* / \partial \alpha$ is positive, and the population mean fitness increases as social association $\alpha$ increases.

We found that if the social association increases, the population mean fitness will increase.
But can an allele that increases the social association evolve? 
To answer this question, we extend our model to include a ``modifier locus''~\citep{Liberman1986,Liberman1988}.
The function of this modifier locus is to determine the social association.
It has no direct effect on fitness.
We assume that the modifier locus has two alleles, $M$ and $m$, which induce association probabilities $\alpha_1$ and $\alpha_2$, respectively, and that initially only $M$ is present.
Suppose that the population evolves to a stable equilibrium when only $M$ is present; we check the local stability of this equilibrium to the introduction of the allele $m$ at the modifier locus. This is called ``external stability''; For more details on external stability, see \citet{Altenberg2017}.
...
\begin{equation} \label{eq:next_gen_p_1}
  \begin{aligned}
	% p1
  \bar{w}\tilde{p}_1' & = \tilde{p}_1(\tilde{p}_1+\tilde{p}_3)(1+b-c)(1 - (1-\alpha_1)(\tilde{p}_2 + \tilde{p}_4)T_B) \\
...
\\
	% p2
  \bar{w}\tilde{p}_2' & = \tilde{p}_2(\tilde{p}_2+\tilde{p}_4)(1 - (1-\alpha_1)(\tilde{p}_1 + \tilde{p}_3)T_A) \\
...
\\
	% p3
  \bar{w}\tilde{p}_3' & = \tilde{p}_3(\tilde{p}_1+\tilde{p}_3)(1+b-c)(1 - (1-\alpha_2)(\tilde{p}_2 + \tilde{p}_4)T_B) \\
...
\\
	% p4
  \bar{w}\tilde{p}_4' & = \tilde{p}_4(\tilde{p}_2 + \tilde{p}_4)(1 - (1-\alpha_2)(\tilde{p}_1 + \tilde{p}_3)T_A) \\
...
%\end{equation}
where $\bar{w} = 1 + (b-c)\cdot(\tilde{p}_1 + \tilde{p}_3)$ is the population mean fitness.% and $\vec{\tilde{p}} = (\tilde{p}_1,\tilde{p}_2,\tilde{p}_3,\tilde{p}_4)$.

The local stability of $\vec{\tilde{p}^*} = (\tilde{p}^*, 1-\tilde{p}^*, 0, 0)$, the equilibrium when only allele $M$ is present (see \autoref{eq:vert_hori_equilibrium}), to the introduction of allele $m$ is determined by the linear approximation $\vec{L}^*$ of the transformation in \autoref{eq:next_gen_p_1} near $\vec{\tilde{p}^*}$ (i.e., the Jacobian of the transformation at the equilibrium).
$\vec{L}^*$ is known to have a block structure, with the diagonal blocks occupied by the matrices $\vec{L}^*_{in}$ and $\vec{L}^*_{ex}$.
The latter is the external stability matrix: the linear approximation to the transformation near $\vec{\tilde{p}^*}$ involving only the phono-genotypes $Am$ and $Bm$. Because we assume that $\vec{\tilde{p}^*}$ is internally stable (i.e. locally stable to small perturbations in the frequencies of $AM$ and $BM$), the stability of 
$\vec{\tilde{p}^*}$ is determined by the eigenvalues of the external stability matrix $\vec{L}^*_{ex}$, which is derived from \autoref{eq:next_gen_p_1} as
\begin{equation}
\mathbf{L}^*_{ex} = \frac{1}{\bar{w}^*} \begin{bmatrix}
...
\end{bmatrix} \;.
\end{equation}
If the eigenvalues of $\mathbf{L}^*_{ex}$ are less than 1, then the equilibrium $\vec{\tilde{p}}$ is externally stable and allele $m$ cannot invade the population of allele $M$. 

\begin{equation} \label{eq:partial_derivatives}
  \begin{aligned}
  &D_{33} = \frac{\partial\bar{w}\tilde{p}_3'}{\partial \tilde{p}_3}(\vec{\tilde{p}}^*) = b(\tilde{p}^*-(1-\alpha_2)T_B(\tilde{p}^*-\tilde{p}^{*2}))+(1-c)(1+\tilde{p}^*T_B-T_B)\\
  &D_{34} = \frac{\partial\bar{w}\tilde{p}_3'}{\partial \tilde{p}_4}(\vec{\tilde{p}}^*) = bT_A\tilde{p}^*(\tilde{p}^*+\alpha_2(1-\tilde{p}^*))+\tilde{p}^*T_A\\
  &D_{43} =\frac{\partial\bar{w}\tilde{p}_4'}{\partial \tilde{p}_3}(\vec{\tilde{p}}^*) = (1-\tilde{p}^*)(1-c)T_B+b\tilde{p}^*(1-\alpha_2)(1-\tilde{p}^*)T_B\\
  &D_{44} =\frac{\partial\bar{w}\tilde{p}_4'}{\partial \tilde{p}_4}(\vec{\tilde{p}}^*) = 1-\tilde{p}^*T_A+ b\tilde{p}^*(1-\alpha_2T_A(1-\tilde{p}^*)-T_A\tilde{p}^*)\\
  \end{aligned}
\end{equation}
Looking on the greater eigenvalue of $\mathbf{L}^*_{ex}$ $\lambda_1$:
\begin{equation} \label{eq:eigenvalues}
  \lambda_1 = \frac{D_{33} + D_{44} + \sqrt{(D_{33}+D_{44})^2-4*(D_{33}+D_{44}-D_{43}D_{34})}}{2} 
\end{equation}
Using SymPy \emph{SymPy} \citep{Meurer2017}, a Python library for symbolic mathematics we get that $\lambda_1 > 1$ when:
\begin{equation} \label{eq:eigenvalues_greater_than_1_condition}
 \alpha_2 > \frac{b\tilde{p}^*(T_B-T_A) + T_B - T_A  + c (1-T_B)}{b*(T_A(1-\tilde{p}^*) + T_B\tilde{p}^*)}
\end{equation}
Using \autoref{eq:vert_hori_equilibrium} we get that $\lambda_1 > 1$ when:
\begin{equation} \label{eq:eigenvalues_greater_than_1_final_condition}
  \alpha_2 > \alpha_1
 \end{equation}
...
\begin{figure}[h]
  ...
  \caption{
  \textbf{Evolution of cooperation in a spatial model and local selection.}
  The expected frequency of cooperators in a structured population after 10,000 generations is shown (red for 0\%, green for 100\%) as function of both the cost of cooperation ($c$) on the y-axis, and the symmetric horizontal transmission rate ($T=T_A=T_B$) on the x-axis of the left panel, or the transmission bias $T_A-T_B$ on the x-axis of the right panel.
  The population evolves on a 100-by-100 grid.
  Cooperation and horizontal transmission are both local between adjacent sites, and each site had 8 neighbors.
...
\begin{figure}[h]
  \centering
   ...
   \caption{
    \textbf{Stable coexistence of both phenotypes in spatial model.}
    The frequancy of cooperators (green) and defactors (red) in the spatial model. 
    Both phenotypes have initial frequency of 50 precent. The figure shows that there are no fixation of nither phenotype through out the simulation.
    The frequency of both cooperators and defactors oscillates around the initial frequency. Here, population size is 10,000 (100-by-100 grid), selection operates globally, 
    benefit of cooperation, $b=1.3$, perfect vertical transmission $v=1$, horizontal transmission of cooperators $T_A = 0.4$, horizontal transmission of defactors $T_B = 0.435$ and cost $c=0.02$.
    }
