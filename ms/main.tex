...
The frequencies of the pheno-genotypes in the next generation are defined by the recursion system, where $x=\tilde{p}_1+\tilde{p}_3$ is the cooperator frequency, 
...
%\end{equation}
where $\bar{w} = 1 + (b-c) x$ is the population mean fitness.% and $\vec{\tilde{p}} = (\tilde{p}_1,\tilde{p}_2,\tilde{p}_3,\tilde{p}_4)$.
...
\begin{equation} 
  \begin{aligned}
	...
	\cdot
	\begin{bmatrix}
	1+b\tilde{p}^*-c & 0 \\ 0 & 1+b\tilde{p}^*
	\end{bmatrix}
	+ 
	\alpha_2 \cdot b \cdot \tilde{p}^* (1-\tilde{p}^*)
	\cdot \vec{T} \;,
  \end{aligned}
\end{equation}
where
