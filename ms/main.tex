...
We define the initial frequency as $\tilde{p}_0$ and
...
\item Fixation of cooperation,
	...
	if $T_A<T_B$ and $\gamma_2 < c$.
\item Global fixation of defection,
	if $T_A>T_B$ and $\gamma_2 < c$, or
	if $T_A<T_B$ and $c < \gamma_1$, or
	if $T_A<T_B$ and $\gamma_1 < c < \gamma_2$, and $\tilde{p}_0<\tilde{p}^*$.
\item Co-existence of both phenotypes at $\tilde{p}^*$, 
	if $T_A<T_B$ and
	$\gamma_2 < c < \gamma_1$.
\end{enumerate}

Importantly, the condition for cooperation to increase from rarity ($\tilde{p}_0 \approx 0$) is
\begin{equation} \label{eq:unequal_transmission_from_rarity}
T_A>T_B \; \text{and} \; c < \gamma_1
\end{equation} 
Thus, cooperation must have a horizontal transmission bias, and the cost of cooperation cannot be too big. 
...
When $\alpha=0$, 
if there is a horizontal transmission bias for cooperation, $T_A>T_B$, the cost is not too high $c < (1+b) \frac{T_A-T_B}{1-T_B}$, and the initial frequency is high enough, $\tilde{p}_0 > \tilde{p}^*$, then cooperation will fix in the population,
...
\end{aligned}\end{equation}
If $T_A>T_B$ then $c>0>\gamma_1$ and cooperation cannot increase from rarity, but it can fix if $c<\frac{(1+b) (T_A - T_B)}{1-T_B}$; otherwise defection will fix.

If $T_A<T_B$ then % TODO check this... gamma2<0 means fixation of A???
\\
