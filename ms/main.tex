...
\begin{equation} \label{eq:oblique_and_vertic_result}
  ...
  \frac{\beta_3}{\beta_1}.
\end{equation}
A detailed explain of how we get eq.~\ref{eq:general_case_polynomial}, eq.~\ref{eq:polynomial_coefficients} and eq.~\ref{eq:oblique_and_vertic_result} can be found in appendix B.
...
\begin{equation} \label{eq:appendix_b_4}\tag{B.4}
  f(\hat{p}) = \bar{w}(\hat{p}'-\hat{p}) =
  \beta_1 \hat{p}^3 + \beta_2 \hat{p}^2 + \beta_3 \hat{p} + \beta_4,
\end{equation}
Where
\begin{equation} \label{eq:appendix_b_5}\tag{B.5}
\begin{aligned}
  \beta_1 &= \big[c(1-v) - b (1-\alpha v)\big] (T_A-T_B) , \\
  \beta_2 &= -\beta_1 -\beta_3 ,  \\
  \beta_3 &= \alpha bvT_A - cv(1-T_B) + (T_A-T_B), \\
...
\begin{equation} \label{eq:appendix_b_6}\tag{B.6}
  \begin{aligned}
    \beta_1 &= 0 , \\
    \beta_2 &= -\beta_3 ,  \\
  ...
  \end{aligned}
\end{equation}
Substituting the coefficients in eq.~\ref{eq:appendix_b_6} in eq.~\ref{eq:appendix_b_4} give us eq.~\ref{eq:equal_horizontal_transmission}.

Now we will prove that $f'(\hat{p}^*)<0$ is a sufficient(?) condition for stability of equilibrium $\hat{p}^*$.
We will write $f(\hat{p})$ as a taylor aproximaiton around the equilibrium $\hat{p}^*$. 
\begin{equation} \label{eq:appendix_b_taylor}\tag{B.7}
...
\begin{equation} \label{eq:appendix_b_taylor0}\tag{B.8}
  f(\hat{p}) = \bar{w}(\hat{p}'-\hat{p}) =  f'(\hat{p}^*)(\hat{p}-\hat{p}^*) + R_2(\hat{p})
\end{equation}
Neglecting the remainder(?) and we get: 
\begin{equation} \label{eq:appendix_b_taylor_without_remainder}\tag{B.9}
  f(\hat{p}) = \bar{w}(\hat{p}'-\hat{p}) = f'(\hat{p}^*)(\hat{p}-\hat{p}^*)
\end{equation}
And we get:
\begin{equation} \label{eq:appendix_b_derivative}\tag{B.10}
  \begin{aligned}
  f'(\hat{p}^*) &= \bar{w}(\frac{\hat{p}'-\hat{p}}{\hat{p}-\hat{p}^*})\\
  & = \bar{w}(\frac{\hat{p}'-\hat{p}^*+\hat{p}^*-\hat{p}}{\hat{p}-\hat{p}}) \\
  & = \bar{w}(\frac{\hat{p}'-\hat{p}^*}{\hat{p}-\hat{p}^*}-1)
  \end{aligned}
\end{equation}
...
In other words, $\frac{\hat{p}'-\hat{p}^*}{\hat{p}-\hat{p}^*}<1$ for every $\hat{p}$. Therefore,
\begin{equation} \label{eq:appendix_b_condition}\tag{B.11}
  \begin{aligned}
  f'(\hat{p}^*) = \bar{w}(\frac{\hat{p}'-\hat{p}^*}{\hat{p}-\hat{p}^*}-1) < 0
  \end{aligned}
\end{equation}
