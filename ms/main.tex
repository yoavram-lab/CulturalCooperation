...
To present this analysis in terms of conditions on the cost of cooperation $c$, we define the cost boundaries, $\gamma_1$ and $\gamma_2$, and the vertical transmission threshold, $\hat v$,
...
We assume a population at a stable coexistence of the two phenotypes, cooperation $A$, and defection $B$, such that the frequency of $A$ among parents is $\tilde{p}^*$ (\autoref{eq:general_equilibrium}).
...
The stable population mean fitness is $\bar{w}^* = 1 + \tilde{p}^*(b-c)$, which is increasing in $\tilde{p}^*$. 
