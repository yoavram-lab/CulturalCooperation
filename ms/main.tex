...
p & > \frac{c\cdot(1-T_B)+T_B-T_A}{b\cdot(T_A-T_B)} .
\end{split}
\end{equation}
Hence, cooperation will take over the population when when eq.~\ref{eq:horizontal_transmission_from_population_reorder} is satisfied, that is if $p$ is above the threshold on the RHS. 
Specifically, a sufficient condition for cooperation fixation is found by setting the RHS to be negative:
\begin{equation} 
