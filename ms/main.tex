...
We model a non-overlapping population colonizing a 2D grid of size 100x100, where each site is inhabited by one individual, similarly to \citet{lewin2020rockpaperscissors}.
...
\begin{figure}[htb]
  ...
  The proportion of cooperators after 10,000 generations is plotted (color coded) as function of the ration between the cost and benefit of cooperation ($c/b$) on the y-axis, and the horizontal transmission rate of both phenotypes $A$ and $B$ (without transmission bias, $T=T_A=T_B$) on the x-axis. The blue curve represents the condition for the evolution of cooperation in a well-mixed population with partner selection, $c/b<\alpha T / (1-T)$(\autoref{eq:equal_transmission}), where $\alpha=1/8$.
  The difference between the results of the spatial model and the analytical condition are because in the spatial model selection is local (i.e. among neighbors, offspring are born to their parent's neighborhood) whereas in the well-mixed population model selection is global.
  Here, population size is $10,000$ (100-by-100 grid); cost of cooperation, $c=0.05$. Simulations were stopped at generation 10,000 or if one of the phenotypes fixed.
  }
  \label{fig:spatial}
\end{figure}
