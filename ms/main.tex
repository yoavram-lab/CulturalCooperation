...
If there is a horizontal transmission bias in favor of cooperation,
...
In this case $0<v<1$, and the equation system is more complex.
...
We start with the simpler case of symmetric horizontal transmission, $T=T_A=T_B$. 
We apply eqs.~\ref{eq:horizontal}, \ref{eq:nextgen_adults_slimpify}, and \ref{eq:nextgen_parents_simplified} to define $f(\hat{p})$ as 
% Yoav: I checked again, substituting eq 7 and 9 to eq 5: you have to divide the RHS by wbar.
...
The equilibria $\hat{p}^*$ are solutions to $f(\hat{p}^*)=0$, or $\hat{p}'^* = \hat{p}^*$.
...
\end{equation}
An equilibrium $\hat{p}^*$ is locally stable when the derivative of $f(\hat{p})$ at $p^*$ is negative, $f'(\hat{p}^*)<0$. 
Since the derivative is 
\begin{equation}
f'(\hat{p})=(1-2\hat{p})\big[\alpha bvT - cv(1-T)\big],
\end{equation}
then the condition for local stability of $\hat{p}^*_1$ is
...
This is the same condition we found in Corollary 1.1, Eq.~\ref{eq:equal_transmission}.
\\
\\
...
By solving $f(\hat{p})/\big[\hat{p}(1-\hat{p})\big] = \beta_3 -\beta_1 \hat{p} = 0$ we can find the third equilibrium,
...
Note that the sign of the cubic $f(\hat{p})$ at positive (negative) infinity is equal (inverse) to the sign of $\beta_1$. 
If $T_A>T_B$, then 
\begin{equation} \label{eq:beta1}
 ...
 \end{equation}
since $c<b$, and $1>\alpha v$.
So the sign of the cubic at positive and negative infinity is negative and positive, respectively.
First, if $\beta_3<\beta_1$ then $0<\hat{p}^*_2<1$ and therefore $f'(0)<0$ and $f'(1)<0$, that is, both fixations are locally stable and $\hat{p}^*_2$ separates the domains of attraction.
Second, if $\beta_1<\beta_3<0$ then $1<\hat{p}^*_2$ and therefore $f'(0)<0$ and $f'(1)>0$, that is, fixation of the defector phenotype $B$ is the only locally stable legitimate (i.e. between 0 and 1) equilibrium.
Third, if $0<\beta_3$ then $\hat{p}^*_2<0$ and therefore $f'(0)>0$ and $f'(1)<0$, that is, fixation of the cooperator phenotype $A$ is the only locally stable legitimate equilibrium.
...
First, if $\beta_3<0$ then $\hat{p}^*_2<0$ and therefore $f'(0)<0$ and $f'(1)>0$, that is, fixation of the defector phenotype $A=B$ is the only locally stable legitimate equilibrium.
Second, if $0<\beta_3<\beta_1$ then $0<\hat{p}^*_2<1$ and therefore $f'(0)>0$ and $f'(1)>0$, that is, both fixations are locally unstable and $\hat{p}^*_2$ is a stable polymorphic equilibrium.
Third, if $\beta_1<\beta_3$ then $\hat{p}^*_2>1$ and therefore $f'(0)>0$ and $f'(1)<0$, that is, fixation of the cooperator phenotype $A$ is the only locally stable legitimate equilibrium.
...
The cultural evolution of a cooperator phenotype will follow on of th following scenarios, depending on the signs of the horizontal transmission bias $T_A-T_B$, the coefficient $\beta_3$, and ratio $\beta_3/\beta_1$:
\begin{enumerate}
