...
$R(\lambda)$ is defined as a determinant, $R(\lambda)=\det(\cl^*_{ex} - \lambda \ci)$, where $\ci$ is the 2-by-2 identify matrix. 
Since multiplication by a positive doesn't change the sign, and using the properties of the determinant, 
...
Combining inequalities~\ref{eq:R1_less} and~\ref{eq:c_gamma1_utility}, we find that $R(1)<0$ if and only if $\alpha_1 > \alpha_2$. This is a sufficient condition for external instability. 
...
However, we numerically validated that the leading eigenvalue is greater than one if and only if $\alpha_1>\alpha_2$.
