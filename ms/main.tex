...
% check this, I think you are missing a wbar on the left side, ie it should be
% \bar{\hat{p}' - \hat{p}} =  \hat{p} ^2(-\alpha bvT + cv(1-T)) + \hat{p} (\alpha bvT -cv(1-T))
...
\end{equation}
We wish to determine when fixation of cooperation is locally stable. % state the general condition on the derivative that implies local stability
...
\begin{equation} \label{eq:general_case_polynomial}% check, because I think wbar is missing
...
\begin{equation} \label{eq:derivative_of_phattag-phat2}
 ...
 \end{equation}
We assume that $T_A>T_B$ and we get that $\beta_1<0$ since $c(1-v)-b(1-\alpha v)<0$ and $c<b$ $v<1$. % why?
In this case the cubic $f(\hat{p})$ goes to infinity when $\hat{p}$ goes to infinity. 
Therefore, if $\hat{p}^*_2$ is positive then $f'(0)$ must be negative % are you sure? I think it must be positive, because i think that the two extremes of a cubic must go in opposite directions
...
Hence, if $\hat{p}^*_2<1$ then $f'(1)$ must be negative % again, i think that the two extremes of a cubic must go in opposite directions
...
We show that when $0<\hat{p}^*_2<1$ hence $\beta_3<0$ and $\beta_1-\beta_3<0$ we get that $-1<\beta_1-\beta_3$: % cut down sentence to several sentences, it is too complex like this
