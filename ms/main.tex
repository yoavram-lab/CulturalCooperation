...
Without association ($\alpha=0$) and with horizontal bias for cooperation ($T_A>T_B$), then 
...
With perfect social association ($\alpha=1$), there are only two equilibria, $\tilde{p}=0$ and $\tilde{p}=1$.
...
\end{equation}
\end{corollary}
With perfect social association horizontal transmission always co-occurs with the cooperative interaction. The same occurs in \citet{lewin2017microbes}, and therefore this corollary is equivalent to their result (see their eq.~1).
...
\item \emph{Stable Coexistence of both phenotypes at $\hat{p}^*$}: 
  if \emph{(vii.a)} $T_A < T_B$ and $v<\hat{v}$ and $c < \hat\gamma_2$ and $c < \hat\gamma_1$; or
...
We constructed an additional model in which partner association emerges from the population structure.
...
Therefore, the effective partner association $\alpha$ in this model is the probability that the same neighbor is picked for both interactions, or $\alpha=1/m$, where $m$ is the number of neighbors. On an infinite grid, $m=8$, but on a finite grid $m$ can be lower in edge neighborhoods close to the grid border.
...
This demonstrates that the partner association parameter $\alpha$ can emerge from local interactions between individuals in a structured population.
...
\begin{figure}[htb]
  \centering       
    \includegraphics[scale=0.5]{Result2_zero_alpha.pdf}

    \includegraphics[scale=0.5]{Result2__non_zero_alpha.pdf}
    
    \includegraphics[scale=0.5]{Result2__alpha_on_y_axis.pdf}
...
(\textbf{c})~the social association $\alpha$ is on the y-axis; the social association boundaries $a_1$ and $a_2$ (\autoref{eq:boundries_assortative_meeting}) are the solid and dashed lines.
...
\begin{figure}[htb]
  \centering       
    \includegraphics[scale=0.5]{Result3_boundaries.pdf}
    \caption{\textbf{Evolution of cooperation under vertical, oblique and horizontal cultural transmission.} 
The figure shows the global fixation of cooperation (green), global fixation of defection (red) and stable coexistence of cooperation and defection (yellow).
In all cases the horizontal bias ($T_A-T_B$) is negative meaning $T_B>T_A$.
The vertical transmission rate $v$ is on the y-axis; the cost boundaries $\hat\gamma_1$ and $\hat\gamma_2$ (\autoref{eq:cost_boundaries_v}) are the solid and dashed lines. 
Here, $b=2$, $T_A=0.5$, $T_B = 0.7$, $\alpha = 0.7$.}
    \label{fig:result3}
\end{figure}
...
\begin{figure}[htb]
  ...
  The black curve represents the condition for the evolution of cooperation in a well-mixed population with partner association, $c/b<\alpha T / (1-T)$, where $\alpha=1/8$; see \autoref{eq:equal_transmission}.
