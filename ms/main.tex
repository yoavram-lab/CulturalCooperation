...
\end{aligned}
\end{equation}
Then, applying Eqs.~\ref{eq:vert_hori_equilibrium}, \ref{eq:vert_hori_global_condition1}, and \ref{eq:vert_hori_global_condition2}, we  summarize the possible outcomes which are illustrated in \autoref{fig:results2}:
...
With horizontal bias for defection ($T_B>T_A$; negative x-axis) cooperation fixes if $alpha$ is above the blue line ($a_2$), but can be maintained in coexistence with defection (yellow) if $\alpha$ is above the orange line ($a_1$). Without horizontal bias ($T_A=T_B$) fixation occurs if $\frac{c}{b} \cdot \frac{1-T}{T} < \alpha$ (inequality~\autoref{}).
...
\begin{figure}[htb]
\centering
\includegraphics{coexistence.pdf}
\caption{\textbf{Stable coexistence between cooperation and defection.} The frequency of the cooperative phenotype $A$ among juveniles in the next generation $\hat{p}'$ is plotted as a function of the frequency in the current generation $\hat{p}$ for $T_A<T_B$ and $\gamma_2<c<\gamma_1$. The line $\hat{p}'=\hat{p}$ is shown as a dashed black line.
The curve and dashed line intersect at the equilibrium $\hat{p}^*$ (black circle). 
When the curve is above the dashed line ($\hat{p}<\hat{p}^*$) the frequency $\hat{p}$increases towards $\hat{p}^*$.
When the curve is below the dashed line ($\hat{p}>\hat{p}^*$) the frequency $\hat{p}$ decreases towards $\hat{p}^*$.
% TODO add the actual values of all parameters.
...
\begin{figure}[htb]
  ...
  % TODO for some reason figs a and b have a lot of white space on their right side
    \includegraphics[scale=0.75]{figure2a.pdf}
    
    \includegraphics[scale=0.75]{figure2b.pdf}
...
The figure shows the global fixation of cooperation (green), global fixation of defection (red), fixation of either cooperation or defection depending on the initial conditions (blue), and stable coexistence of cooperation and defection (yellow).
...
Here, $b=1.3$, $T_A=0.4$.
  % TODO what is c in (3) and alpha in (1) and (2)??
  % TODO missing text descriptions of conditions on (3)
  % TODO change lines from blue and orange to black solid and black dashed, and make them a bit wider
  % TODO I think you were right: in the figure we should call it stable coexistence and unstable coexistence.
  	}
    \label{fig:results2}
\end{figure}
